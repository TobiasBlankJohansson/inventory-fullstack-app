  ...
  (id: string,
   ...
   setEdit: React.Dispatch<React.SetStateAction<boolean>>) => {
    ...
    const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      const newItemData = toItemFromFormField(Object.fromEntries(
        FORM_FIELDS_ITEM.map(({key}) =>
          [key, new FormData(e.currentTarget).get(`item_${key}`)])
      ) as FormFieldItem, equipments, item.id);
      ...
      if (item.responsible != newItemData.responsible || item.equipment.name != newItemData.equipment.name) {
        if (items.some(
          (item) =>
            item.equipment.name === newItemData.equipment.name && item.responsible === newItemData.responsible
        )) {
      ...
      const itemDto: putItemDto = {
        ...
        storageId:
          storageFromList(newItemData.storageArea, storageArea)?.id ?? "",
      };
      console.log(itemDto);
